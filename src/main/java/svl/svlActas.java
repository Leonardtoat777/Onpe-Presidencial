...
public class svlActas extends HttpServlet {
	...
	protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Object data = null; //, departamentos = null, provincias = null;
		...
		if (idProvincia != null && idProvincia.equals("-1")) {
			idDistrito ="-1";
			session.setAttribute("distrito",null);
		}
		if (idDistrito != null && idDistrito.equals("-1")) {
			idLocalVotacion ="-1";
			session.setAttribute("LocalesVotacion",null);
		}
		...
		if ( idDistrito.equals("-1")) {
			session.setAttribute("LocalesVotacion", daoOnpe.getLocalesVotacion(idDistrito));
		}
		
		if  (!idLocalVotacion.equals("-1")) {
			session.setAttribute("mesas", daoOnpe.getGrupoVotacion(idLocalVotacion));
		}
